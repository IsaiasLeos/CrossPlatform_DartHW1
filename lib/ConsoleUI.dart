...
class ConsoleUI {
  ...
  void showUI() async {
    ...
    stdout.write('\nWelcome!\n');
    stdout.write('What quiz would you like to take (1-4)?');
    ...
    quiz =
        await QuizController.question_repo(repo); //initialize quiz controller
    await quiz.manage(int.parse(input));
    await quiz.manageQuestions();
    stdout.write('Total Grade: ');
    stdout.write(
        (await quiz.calculateGrade() / valueOfGrade) * 100); //calculate grade
    stdout.write('\n');
  }
}
